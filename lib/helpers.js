...
function getJsonDirectoryPath() {
  return nuxtConfig.gustave.jsonDirectory
    ? nuxtConfig.gustave.jsonDirectory
    : 'static/api'
}
...
function saveToJsonDirectory(filepath, data) {
  const basePath = getJsonDirectoryPath()
...
function runImporters() {
  ...
  console.log(
    '\x1b[34m',
    `🗑️  Gustave: remove all files from "${getJsonDirectoryPath()}" directory`
  )

...
function parseMarkdownDirectory(inputDirectory, options = {}) {
  let markdownItInstance = options.markdownIt || null
  const directoryPath = inputDirectory
  const files = fs.readdirSync(directoryPath)
  // use a default markdownInstance if none is passed
  if (markdownItInstance === null) {
    markdownItInstance = require('markdown-it')({
      html: true,
      linkify: true
    })
  }
  const resources = files
    ...
    .map(filename => {
      ...
      resource.$slug = slug(filename.substring(0, filename.length - 2))
      return resource
    })
  return resources
}
